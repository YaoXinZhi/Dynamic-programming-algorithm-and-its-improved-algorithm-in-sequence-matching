...
def generate_matrix_BAND(seq_list: list, simarty_matrix: dict, indel_score:int, band_B: int):
    if len(seq_list) != 2:
    ...
    for i in range(1, min(x, y)):
        for j in range(i, min(i+band_B+1, y)):
            ...
            if max(score_up, score_left, score_skew) == score_up:
                ...
                cell_label[position] = 'S'
        for j in range(i, min(i+band_B+1, x)):
            score_up = distance_matrix[j-1, i] + indel_score
            score_left = distance_matrix[j, i-1] + indel_score
            score_skew = distance_matrix[j-1, i-1] + simarty_matrix[(seq1[j-1], seq2[i-1])]
            distance_matrix[j, i] = max(score_up, score_left, score_skew)
            position = (j, i)
...
if __name__ == '__main__':
    ...
    distance_matrix, cell_label, entries = generate_matrix_BAND(seq_list, simarty_matrix, indel_score, band_B)
