...
def generate_matrix_X_drop(seq_list: list, simarty_matrix: dict, indel_score: int, drop_X: int):
    ...
    flag = True
    d = 0
    antidiagonal_dict = defaultdict(list)
    consistent_list = []
    not_consistent_list = []
    distance_matrix[0, 0] = 0
    antidiagonal_dict[0].append(0)
    consistent_list.append((0,0))
    Tau = max(antidiagonal_dict[d])
    for d in range(1, len(seq1)+len(seq2)):
        if d > 2:
            flag = False
            d_1_list = d_to_index(d-1)
            d_2_list = d_to_index(d-2)
            for i in d_1_list + d_2_list:
                if i in consistent_list:
                    flag = True
        if not flag:
            entries = len(consistent_list)+len(not_consistent_list)
            return distance_matrix, cell_label, entries
        for i in range(0, d+1):
            j = d-i
            if i >= x or j >= y:
                continue
            if i == 0 and (i, j-1) in consistent_list:
                distance_matrix[i, j] = distance_matrix[i, j-1] - 1
            elif j == 0 and (i-1, j) in consistent_list:
                distance_matrix[i, j] = distance_matrix[i-1, j] - 1
            else:
                if (i-1, j-1) not in consistent_list and (i-1, j) not in consistent_list and (i, j-1) not in consistent_list:
                    continue
            ...
            antidiagonal_dict[d].append(distance_matrix[i, j])
            if distance_matrix[i, j] >= Tau - drop_X:
                consistent_list.append((i, j))
            else:
                not_consistent_list.append((i, j))
        if antidiagonal_dict[d]:
            Tau = max(Tau, max(antidiagonal_dict[d]))
    entries = len(consistent_list)
    return distance_matrix, cell_label, entries
...
def Maximum_backtracking(distance_matrix, cell_label: dict, seq_list: list):
...
if __name__ == '__main__':
    ...
    distance_matrix, cell_label, entries = generate_matrix_X_drop(seq_list, simarty_matrix, indel_score, drop_X)
    # print_matrix(distance_matrix, seq_list)
    seqA_list, seqB_list = Maximum_backtracking(distance_matrix, cell_label, seq_list)
